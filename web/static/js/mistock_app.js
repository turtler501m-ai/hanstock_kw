...
function renderHoldingAccountSummary(balance, displayTotal, realizedPnl = 0) {
    ...
    const source = balance._cache?.stale
        ...
        : (sourceMap[balance.balance_source] || '미스톡 계좌정보');
    ...
    rows.forEach((holding) => {
        ...
        tr.innerHTML = `
            <td>
                ...
                <div class="symbol-code">${escapeHtml(holding.symbol)} ${sourceBadge}</div>
            </td>
            <td>${qtyText}</td>
...
async function renderBalance() {
    try {
        ...
        const accountReturnEl = setElementText('val-account-return', accountReturnRate == null ? '-' : formatPercent(accountReturnRate));
        if (accountReturnEl) {
