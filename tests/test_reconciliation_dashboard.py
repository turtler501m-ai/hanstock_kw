...
class ReconciliationDashboardTests(unittest.TestCase):
    ...
    def insert_issue(self, *, broker_qty=7, internal_qty=10):
        with stock_order.trader.connect_db() as conn:
            ...
            conn.execute(
                ...
                (broker_qty, internal_qty, broker_qty - internal_qty),
            )
    ...
    def test_apply_refuses_stale_recorded_broker_quantity(self):
        ...
        with stock_order.trader.connect_db() as conn:
            self.assertEqual(
                conn.execute("SELECT status FROM reconciliation_adjustments").fetchone()[0],
                "open",
            )
